...
export class ApiKeysService {
   ...
   * 创建API密钥
   */
  async create(createApiKeyDto: CreateApiKeyDto, botId: string, userId: string, userRole: string) {
    // 验证机器人是否存在且用户有权限
   ...
   * 更新API密钥
   */
  async update(id: string, updateApiKeyDto: UpdateApiKeyDto, userId: string, userRole: string) {
    const apiKey = await this.prisma.apiKey.findUnique({
      where: { id },
      include: {
        bot: {
