...
import { Card, Avatar, Typography, Tag, Button, Row, Col, Spin, Empty } from 'antd';
...
const BotSelector: React.FC<BotSelectorProps> = ({ onSelectBot }) => {
  ...
  useEffect(() => {
    const fetchBots = async () => {
      try {
        const data = await botsAPI.getPublicList();
        setBots(data);
      } catch (error) {
              ...
              <Card.Meta
                title={
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <Tag
                      color={categoryColors[bot.category as keyof typeof categoryColors]}
                      style={{ marginTop: '8px' }}
                    >
                      {categoryNames[bot.category as keyof typeof categoryNames]}
                    </Tag>
                  </div>
                }
                description={
